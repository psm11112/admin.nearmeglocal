...
class ListController extends ResponseController
{
    ...
    public function getListByField($categoryId,$field,$id){
        $list=ListModel::with(['parent_category','states','city','area','user'])
            ->where('category_id',$categoryId)
            ->where($field,$id)
            ->where('item_status',2)->paginate(config('service.pagination'));
